<body>
                          ...
                          <tbody>
                            ...
                            <?php foreach ($data_category as $category) : ?>
                              <tr>
                                <th scope="row"><?= $no; ?>.</th>
                                <td>
                                  <input type="hidden" id="id_kategori" name="id_kategori" value="<?= $category['id_kategori']; ?>">
                                  <img id="preview_logo" src="<?= base_url('assets/images/logo/' . $category['logo']) ?>" width="200">
                                </td>
                                <td><?= $category['nama_kategori'] ?></td>
                                <td class="text-center">
                                  <button type="button" class="btn btn-sm btn-success btn-burger" data-bs-toggle="modal" data-bs-target="#modalEdit<?php echo $category['id_kategori']; ?>"><i class="material-icons">edit</i></button>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="row mb-3">
            <label for="logo" class="col-sm-2 col-form-label">Logo Kategori</label>
  ...
  foreach ($data_category as $category) :  ?>
    <div class="modal fade" id="modalEdit<?= $category['id_kategori'] ?>" tabindex="-1" aria-labelledby="modalEditLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
          ...
          <div class="modal-body">
            <form action="<?= base_url('category/edit_data'); ?>" method="post" enctype="multipart/form-data" id="myForm">
              <div class="row">
                <label for="nama_kategori" class="form-label">Nama Kategori</label>
                <input type="hidden" name="id_kategori" id="id_kategori" value="<?= $category['id_kategori'] ?>">
                <input type="text" class="form-control" name="nama_kategori" id="nama_kategori" value="<?= $category['nama_kategori'] ?>" required>
              </div>
              <div class="row mt-3">
  ...
  <script>
    $("button[name='btnSubmit']").click(function(e) {
      e.preventDefault();
      let nama_kategori = $("input[name=nama_kategori]").val().trim();
